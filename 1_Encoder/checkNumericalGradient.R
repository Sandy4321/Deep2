checkNumericalGradient <- function() {
...
a = 1 # for debug a function call    
x = c(4, 10);
sQF= simpleQuadraticFunction(x,a);
value = sQF$cost;
grad = sQF$grad;
...
numgrad = computeNumericalGradient(x, simpleQuadraticFunction, a=1);
...
print(diff); 
cat('Norm of the difference between numerical and analytical gradient (should be < 1e-9)\n\n');
}

simpleQuadraticFunction <- function(x,a) {
    ...
    return(list(cost=value, grad=grad))
}